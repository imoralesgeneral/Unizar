...
{
    if (argc < 3) {
        printf("params: <path> <is decrypt mode> [*key]\n");
        system("pause");
        return 0;
    }
    wchar_t *path = argv[1];
   ...
   /* // Imprimir la clave AES-128 generada
    printf("Clave AES-128 generada:\n");
    for (size_t i = 0; i < key_size; ++i) {
        printf("%02X ", (unsigned int)(unsigned char)key_str[i]);
    }
    printf("\n");
    for (size_t i = 0; i < key_size; ++i) {
        printf("%c", key_str[i]); // Imprimir cada carácter en ASCII
    }
    printf("\n");

    // Imprimir el hash
    wprintf(L"Valor de hash: %s\n", key_str);*/

    if (isDecrypt) {
        printf("DECRYPTING...\n");
        std::vector<std::string> textFiles = retrieveEncodedFiles(folderPathStr);
